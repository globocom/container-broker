...
RSpec.describe SlotsUsagePercentage, type: :service do
  let(:available_slot1) { Fabricate(:slot, name: "slot1") }
  let(:available_slot2) { Fabricate(:slot, name: "slot2") }
  ...
  context "with busy and idle slots" do
    context "and decimal result" do
      let(:slots) { [available_slot1, available_slot2, busy_slot1] }
    ...
    context "and integer result" do
      let(:slots) { [available_slot1, busy_slot1] }
  ...
  context "with only available slots" do
    let(:slots) { [available_slot1, available_slot2] }
