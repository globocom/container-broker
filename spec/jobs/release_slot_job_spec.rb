...
RSpec.describe ReleaseSlotJob, type: :job do
  ...
  let(:slot) { Fabricate(:slot_releasing, node: node, current_task: task, runner_id: runner_id, execution_type: "test") }
  ...
  context "when update task status raises and error" do
    ...
    it "raises the error" do
      expect { subject.perform(slot: slot, runner_id: runner_id) }.to raise_error(Excon::Error)
    end
  ...
  context "when the container id is different than the current on slot" do
    it "raises an error" do
      expect do
        subject.perform(slot: slot, runner_id: SecureRandom.uuid)
  ...
  context "when slot needs to be removed" do
    ...
    it "doesn't change the status" do
      expect { subject.perform(slot: slot, runner_id: runner_id) }.to_not change(slot, :status)
    end
